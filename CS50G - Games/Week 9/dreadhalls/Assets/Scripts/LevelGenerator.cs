...
public class LevelGenerator : MonoBehaviour {
	...
	// number of holes we want in the floor
	public int numberOfHoles = 4;
	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				//Random.Range part makes it so they don't just spawn consecutively 
				//z parts make it so they don't spawn at outer walls of the maze since we can't walk there
				if(numberOfHoles!=0 && Random.Range(0,10)>8 && z>0 &&z<mazeSize-1)
				{
